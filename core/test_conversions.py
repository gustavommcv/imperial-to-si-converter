...
def test_length_conversions():
    # 1 in = 2.54 cm
    assert math.isclose(
        convert("Comprimento", "Polegada (in)", "Centímetro (cm)", 10), 25.4
    )
    # 1 ft = 0.3048 m
...
def test_temperature_conversions():
    # 32F = 0C
    assert math.isclose(
        convert("Temperatura", "Fahrenheit (°F)", "Celsius (°C)", 32), 0.0
    )
    # 212F = 100C
    assert math.isclose(
        convert("Temperatura", "Fahrenheit (°F)", "Celsius (°C)", 212), 100.0
    )
