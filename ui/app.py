...
class CalculatorApp(EventsMixin, KeybindsMixin, App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            yield Label(
                "Atividade Semestral 01 - Selecione a Grandeza", id="title-label"
            )

            # Carrega listas limpas
            categories_options: list[tuple[str, str]] = [
                (str(c), str(c)) for c in get_categories()
            ]
            yield Select[str](
                categories_options, id="category-select", prompt="Escolha a Grandeza"
            )
            ...
            with Horizontal(id="conversion-container"):
                ...
                with Vertical(classes="column"):
                    ...
                    yield Select[str](
                        [],
                        id="from-unit-select",
                        prompt="Unidade Origem",
                        disabled=True,
                    )
                    yield Input(
                        placeholder="Digite o valor", id="input-value", type="number"
                    )
                ...
                with Vertical(classes="column"):
                    ...
                    yield Select[str](
                        [], id="to-unit-select", prompt="Unidade Destino", disabled=True
                    )
                    yield Input(
